...
func main() {
	...
	for i := 0; i < k/l - x; i++ {
	ciphertext = cvec1[i].CopyNew()
	if err := eval.Mul(ciphertext, 1 << y, ciphertext); err != nil {
		panic(err)
	}
	if i != 0 {
		cvec[i + x] = ciphertext2.CopyNew()
	}
	...
	Bootstrap()
	if i != 0 {
	if err = eval.Evaluator.Add(ciphertext, cvec[i + x], cvec[i + x]); err != nil {
		panic(err)
	}
	} else {
		cvec[i + x] = ciphertext.CopyNew()
	}
	if i != (k/l - x - 1) {
