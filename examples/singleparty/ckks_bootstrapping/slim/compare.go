...
func main() {
	...
	vecWant[i] = complex(float64(largeWant[i]),0)
	}
	...
	vecTest := printDebug(params, cres, vecWant, decryptor, encoder)
		for j := 0; j < cres.Slots(); j++ {
			tmp := vecTest[j]
			this_err_real := math.Abs(real(tmp) - real(vecWant[j]))
			this_err_imag := math.Abs(imag(tmp) - imag(vecWant[j]))
			this_err := math.Sqrt(this_err_real * this_err_real + this_err_imag * this_err_imag)
			mean_err += this_err
			max_err = math.Max(max_err, this_err)
		}
	mean_err /= float64(cres.Slots())
