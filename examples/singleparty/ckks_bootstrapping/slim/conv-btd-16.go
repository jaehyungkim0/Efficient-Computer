...
func (c *REDCComputer) ConvBtD(ctCombined *rlwe.Ciphertext, modulus, modulusInv []*rlwe.Ciphertext, zero *rlwe.Ciphertext) (out, decomposed, q, u []*rlwe.Ciphertext, flag *rlwe.Ciphertext, bootCount int, err error) {
...
func main() {
	...
	for iter := 0; iter < numIter; iter++ {
		mVals := make([]*big.Int, params.MaxSlots())
		combined := make([]*big.Int, params.MaxSlots())
		want := make([]*big.Int, params.MaxSlots())
		qWant := make([]*big.Int, params.MaxSlots())
		uWant := make([]*big.Int, params.MaxSlots())
		K := int64(2)
		slotValues := make([]complex128, params.MaxSlots())
		for i := range combined {
			mVals[i], err = rand.Int(rand.Reader, modulus)
			if err != nil {
				panic(err)
			}
			iRand, err := rand.Int(rand.Reader, big.NewInt(2*K+1))
			if err != nil {
				panic(err)
			}
			iRand.Sub(iRand, big.NewInt(K))

			lifted := new(big.Int).Mul(modulus, iRand)
			lifted.Add(lifted, mVals[i])
			lifted.Add(lifted, new(big.Int).Mul(modulus, big.NewInt(K)))

			combined[i] = new(big.Int).Set(lifted)
			qWant[i], uWant[i], want[i] = standardMontgomeryREDC(combined[i], modulus, R, modulusInv)
			scaledInput := new(big.Float).SetInt(combined[i])
			scaledInput.Quo(scaledInput, inputNormalizer)
			scaledFloat, _ := scaledInput.Float64()
			slotValues[i] = complex(scaledFloat, 0)
		}

		slotPt := ckks.NewPlaintext(params, combinedLevel)
		if err := encoder.Encode(slotValues, slotPt); err != nil {
			panic(err)
		}
		cSlots, err := encryptor.EncryptNew(slotPt)
		if err != nil {
			panic(err)
		}
		traceSlotsCiphertext("input/slots", cSlots, 8)
		cCombined, err := eval.SlotsToCoeffs(cSlots, nil)
		if err != nil {
			panic(err)
		}
		cCombined.IsBatched = false
		traceCoeffsCiphertext("input/after-StC", cCombined, 8)

		start := time.Now()
		out, decomposed, qDigits, uDigits, flag, bootCount, err := computer.ConvBtD(cCombined, cModulus, cModulusInv, cZero[0])
		if err != nil {
			panic(err)
		}
		elapsed := time.Since(start)
		totalOperationTime += elapsed
		fmt.Printf("ConvBtD time: %s\n", elapsed)
		...
		exactDecomp, maxDecompErr := integerMetrics(decompDecoded, combined, computer.limbBits)
		...
		fmt.Printf("Exact decomposition matches: %d/%d, max |decomp-combined|=%s\n", exactDecomp, len(combined), maxDecompErr.String())
