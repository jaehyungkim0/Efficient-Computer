...
func (c *GoldilocksComputer) ReduceGoldilocks(t, modulus []*rlwe.Ciphertext, zero *rlwe.Ciphertext) ([]*rlwe.Ciphertext, *rlwe.Ciphertext, int, error) {
	...
	for i := 0; i < c.limbCount-1; i++ {
		low[i] = t[i].CopyNew()
	}
	addHigh[8] = t[c.limbCount-1].CopyNew()
	subHigh[0] = t[c.limbCount-1].CopyNew()
	...
	// Since 16^16 = 2^64 ≡ 2^32 - 1 = 16^8 - 1 mod p, the top nibble folds into limbs 8 and 0.
...
func main() {
	...
	computer, err := NewGoldilocksComputer(params, eval, evk, 4, 68)
	if err != nil {
		panic(err)
	}
	...
	limit := new(big.Int).Mul(modulus, big.NewInt(5))

	fmt.Printf("Running specialized reduction for Goldilocks prime p=%s using 2^64 ≡ 2^32 - 1 mod p, beta=%d\n", modulus.String(), computer.mods)
	...
	for iter := 0; iter < numIter; iter++ {
		...
		for i := range T {
			...
			want[i] = referenceReduceGoldilocks(T[i], modulus)
		}

		// Since 5p < 2^67, every T in this example fits in exactly 17 base-16 limbs.
		tDigits := computer.decomposeBig(T, computer.limbCount)
		...
		start := time.Now()
		out, flag, bootCount, err := computer.ReduceGoldilocks(cT, cModulus, cZero[0])
		if err != nil {
			panic(err)
		}
